...
export const processMessage = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // Fetch recent messages for conversation context (reduced to 5 to save tokens)
    ...
    // Title generation disabled to save API calls
    // Users can manually rename conversations if needed
    // Uncomment below if you want to re-enable (costs 1 extra API call per new conversation)
    /*
    ...
    const codingAgent = createAgent({
      ...
      model: gemini({
        ...
        defaultParameters: {
          generationConfig: {
            temperature: 0.3,
            maxOutputTokens: 8000  // Reduced from 16000 to save tokens
          }
        }
      }),
      tools: [
