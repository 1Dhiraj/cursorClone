...
export const exportToGithub = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    const { data: repo } = await step.run("create-repo", async () => {
      // Sanitize description to remove control characters that GitHub rejects
      const sanitizedDescription = description
        ? description.replace(/[\x00-\x1F\x7F]/g, (char) => {
          // Replace newlines and tabs with spaces, remove others
          return char === "\n" || char === "\r" || char === "\t" ? " " : "";
        }).trim()
        : null;
      ...
      return await octokit.rest.repos.createForAuthenticatedUser({
        name: repoName,
        description: sanitizedDescription || `Exported from Polaris`,
